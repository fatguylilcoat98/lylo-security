...
function ChatInterface({
  ...
  // Notification permission state for mobile reminders
  ...
  useEffect(() => {
    ...
    // Hydrate notification permission state
  ...
  // ── v28.0: ACTION TRIGGER WATCHER ────────────────────────────────────────
  // Watches messages for any new bot message carrying an actionTrigger.
  // Fires the moment the JSON lands — does NOT wait for typewriter to finish.
  // scroll-into-view ensures the action button is always visible on arrival.
  useEffect(() => {
    const lastBot = [...messages].reverse().find(m => m.sender === 'bot');
    if (!lastBot || !(lastBot as any).actionTrigger) return;
    // If typewriter is still running, scroll anyway — button will appear on completion
    // If typewriter is done, button is already rendered — scroll to it immediately
    requestAnimationFrame(() => {
      if (chatContainerRef.current) {
        chatContainerRef.current.scrollTop = chatContainerRef.current.scrollHeight;
      }
    });
  }, [messages, streamingMsgId]);  // re-fires when streamingMsgId clears (typewriter done)
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        actionTrigger: response.action_trigger ?? null,
      }]);
      setLoading(false);
  ...
  // ── v28.0: MOBILE ALERTS — Notification permission request ───────────────
  const requestMobileAlerts = async () => {
    ...
    if (permission === 'granted') {
      ...
      new Notification('LYLO OS Alerts Active 🛡️', {
        body: 'Mission reminders and accountability alerts are now enabled.',
        icon: '/icon-192.png',
      });
    } else {
  ...
  // ── v28.0: SCHEDULE REMINDER — fires a Notification after delay ──────────
  // delay: 30 minutes default for accountability actions
  const scheduleMobileReminder = (message: string, delayMinutes: number = 30) => {
    if (!notificationsEnabled || Notification.permission !== 'granted') {
      requestMobileAlerts().then(() => {
        if (Notification.permission === 'granted') {
          setTimeout(() => {
            new Notification('⏰ LYLO Reminder', { body: message, icon: '/icon-192.png' });
    ...
    new Notification(`✅ Reminder Set — ${delayMinutes} min`, {
      body: `I'll check back in ${delayMinutes} minutes: "${message.slice(0, 80)}..."`,
      icon: '/icon-192.png',
    });
  };

  // ── v28.0: EMAIL DISPATCH — manual dispatch from action button ────────────
  // User can tap the 🛡️ button on any bot message to dispatch it as a tactical report
  const handleEmailDispatch = async (content: string) => {
    try {
      ...
      if (res.ok) {
        alert('🛡️ Tactical Report dispatched to your email.');
      } else {
        // Fallback: open mailto with content
        const subject = encodeURIComponent(`LYLO Tactical Report — ${activePersona.name}`);
    ...
    <div className="fixed inset-0 bg-black flex flex-col h-screen w-screen overflow-hidden font-sans z-[99999]">
      ...
      <div className="bg-black/90 border-b border-white/10 p-3 flex-shrink-0 z-50">
        <div className="flex items-center justify-between">
          ...
          {/* Right: user chip + notification bell + crisis button */}
          <div className="flex items-center gap-2 z-10">
            ...
            {/* Notification bell — toggles mobile alerts */}
            <button
              onClick={requestMobileAlerts}
              title={notificationsEnabled ? 'Alerts Active — tap to test' : 'Tap to enable mission alerts'}
              className={`p-3 rounded-xl transition-all ${
                notificationsEnabled
                  ? 'bg-indigo-500/20 border border-indigo-500/40 text-indigo-400 hover:bg-indigo-500 hover:text-white'
