...
_PROMPT_LEAKAGE = re.compile(
    r"(SYSTEM PRIORITY\s*:?\s*|SYSTEM NOTE\s*:?\s*|SECURE THE PERIMETER\s*:?\s*|DIGITAL PERIMETER\s*:?\s*|THREAT DETECTED\s*:?\s*|GUARDIAN ALERT\s*:?\s*|CLINICAL NOTE\s*:?\s*|LEGAL NOTE\s*:?\s*|SAFETY NOTE\s*:?\s*|IMPORTANT\s*:?\s*(?=\w)|"
