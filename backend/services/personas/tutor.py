...
PERSONA_STRING = (
    ...
    "[TUTOR_STATE: {\"phase\": \"EXPLAIN\", \"urgency\": 1, \"subject\": \"algebra\"}]\n"
    "Do not add any text after this block.\n"
    "━━━ END TUTOR STATE PROTOCOL ━━━"
)
...
def inject_fortress(system_prompt: str, msg: str, convo_context: dict,
    ...
    if sig_crisis and not recent_turns:
        ...
        logger.warning("📚 Tutor CRISIS gate fired — urgency 4")

    # Gate 2: Directive / fast-track mode
    elif sig_dir:
        overrides["directive"] = {"en": _DIRECTIVE_EN, "es": _DIRECTIVE_ES}
        logger.info("📚 Tutor DIRECTIVE gate fired")
    ...
    if sig_confusion and recent_turns:
        ...
        logger.info("📚 Tutor confusion rephrase gate injected")
    ...
    if sig_amb and recent_turns:
        ...
        if lu or lr:
            ...
            logger.info("📚 Tutor ambiguous reference context injected")
...
def apply_gates(answer: str, overrides: dict, lang: str) -> str:
    is_es = (lang == "es")
    answer = re.sub(r'\[TUTOR_STATE:.*?\]', '', answer, flags=re.DOTALL).strip()

    if overrides.get("emergency"):
        answer = overrides["emergency"]["es" if is_es else "en"]
        logger.warning("📚 Tutor crisis override applied")
        return answer

    if overrides.get("directive"):
        answer = overrides["directive"]["es" if is_es else "en"]
        logger.info("📚 Tutor directive override applied")
        return answer
