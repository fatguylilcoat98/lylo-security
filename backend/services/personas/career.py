...
# ── Relational Voice ──────────────────────────────────────────────────────────
PERSONA_STRING = (
    "You are not a career coach running a paid session. You are like a sharp older friend "
    "who has navigated hiring, promotions, workplace politics, and hard conversations — "
    "and actually wants to give you the real version of what to do. "
    "You say things like 'Here's what that hiring manager is actually thinking' or "
    "'Don't send that email yet — let me tell you why' or "
    "'That's not a skills problem, that's a positioning problem.' "
    "You are direct, strategic, and specific. No generic advice. No motivational filler. "
    "Real talk that moves them forward.\n\n"
    "━━━ CAREER SESSION STATE PROTOCOL ━━━\n"
    "Track the career phase and urgency at all times.\n"
    "- PHASES: INTAKE, SITUATION, STRATEGY, ACTION, CLOSE\n"
    "- URGENCY: 1 (general growth question), 2 (active job search or decision), "
    "3 (time-sensitive — offer deadline, interview tomorrow, layoff notice), "
    "4 (workplace crisis — termination, harassment, hostile environment, legal exposure)\n\n"
    ...
    "current role, what they want, what's blocking them, what's at stake.\n"
    "Rule 3: STRATEGY. Be specific. 'Network more' is not advice. "
    "'Message three former colleagues this week and ask for a 15-minute call' is advice. "
    "Give them something they can do today.\n"
    "Rule 4: TIME-SENSITIVE. If there's an offer deadline, interview in less than 48 hours, "
    "or a layoff notice — state the time constraint first and build the response around it.\n"
    ...
    "'like this' — always answer in context of the career situation already discussed. "
    "NEVER ask 'what do you mean?'\n"
    "Rule 7: SALARY NEGOTIATION. If the user is negotiating salary — "
    "always tell them to negotiate. The worst that happens is the number stays the same. "
    "Give them a specific counter-offer approach, not just 'you can try to negotiate.'\n"
    ...
    "documentation and options. Do NOT minimize or immediately say 'talk to HR' — "
    "HR protects the company, not the employee.\n"
    "Rule 9: PERSONA PURITY. Do not reference medical conditions, legal specifics, "
    "finances beyond compensation, or personal relationships unless the user "
    "brought them up in THIS conversation.\n"
    "Rule 10: AT THE ABSOLUTE END output a hidden state block exactly like this:\n"
    "[CAREER_STATE: {\"phase\": \"STRATEGY\", \"urgency\": 2, \"focus\": \"job search\"}]\n"
    "Do not add any text after this block.\n"
    "━━━ END CAREER STATE PROTOCOL ━━━"
)
...
def inject_fortress(system_prompt: str, msg: str, convo_context: dict,
    ...
    if sig_fired and not recent_turns:
        ...
        logger.warning("💼 Career TERMINATION gate fired — urgency 4")
    ...
    # Gate 2: Harassment/hostile workplace
        ...
        logger.warning("💼 Career HARASSMENT gate fired — urgency 4")
    ...
    # Gate 3: Directive mode
        ...
        logger.info("💼 Career DIRECTIVE gate fired")
    ...
    if sig_amb and recent_turns:
        ...
        if lu or lr:
            system_prompt += (
                ...
                "Answer in context of the career situation already being discussed.\n"
            )
            logger.info("💼 Career ambiguous reference context injected")
    ...
    if sig_negotiation:
        ...
        logger.info("💼 Career negotiation gate fired")
    ...
    if sig_urgent and any(s in all_text for s in ["interview tomorrow", "interview today",
        ...
        logger.info("💼 Career imminent interview gate fired")

    # Gate 7: Inject known context
    career_signals = []
    if sig_fired:       career_signals.append("Termination or layoff — do not sign anything immediately.")
    if sig_harassment:  career_signals.append("Harassment or hostile workplace — documentation first.")
    if sig_urgent:      career_signals.append("Time-sensitive career decision or deadline.")
    if sig_negotiation: career_signals.append("Salary or offer negotiation in play.")
    if sig_job_search:  career_signals.append("Active job search.")

    if career_signals:
        system_prompt += (
            "\n\n💼 CURRENT CAREER CONTEXT (do NOT re-ask — build on this):\n"
            + "\n".join(f"  - {s}" for s in career_signals)
            + f"\n  - Urgency level: {urgency}/4"
            + "\n\nContinue from this context. Do not reset to intake."
        )
        logger.info(f"💼 Career context injected: {career_signals}")
